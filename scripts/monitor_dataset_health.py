#!/usr/bin/env python3
"""
Dataset Health Monitor CLI
===========================
Executes the dataset health pipeline checks and generates a detailed report.
"""
...
# Setup health reporting directory
HEALTH_REPORTS_DIR = REPORTS_DIR / "dataset_health"
ensure_dirs(LOGS_DIR, HEALTH_REPORTS_DIR)

# Initialize logger
logger = logging.getLogger("dataset_health_monitor")
...
# File handler
fh = logging.FileHandler(LOGS_DIR / "dataset_health.log", mode="a", encoding="utf-8")
...
def write_health_report(
    ...
    """
    Generate and save a detailed dataset health report in markdown format.
    """
    lines = [
        "# Dataset Health Monitoring Report",
        "",
        "This report provides a detailed health summary of the NEU Metal Surface Defects dataset.",
        "",
    ...
    lines.append("## 4. Image Duplication Check")
    lines.append("")
    lines.append(f"- **Duplicate image groups (Same MD5)**: {len(duplicates)}")
    if duplicates:
    ...
    lines.append("## 5. Cross-Split Stem Duplicates (Data Leakage)")
    lines.append("")
    ...
    lines.append("---")
    lines.append("Report generated automatically by `monitor_dataset_health.py`.")

    with open(report_path, "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    logger.info("Successfully generated dataset health report at: %s", report_path)


def main() -> None:
    parser = argparse.ArgumentParser(description="Run Automated Dataset Health Monitor.")
    parser.add_argument(
        "--report-dir",
        type=str,
        default=str(HEALTH_REPORTS_DIR),
        help="Directory to save the health report."
    )
    args = parser.parse_args()
    report_dir = Path(args.report_dir)
    ensure_dirs(report_dir)

    logger.info("Starting automated dataset health monitoring pipeline...")
    ...
    report_path = report_dir / "dataset_health_report.md"
    write_health_report(
        report_path,
        folder_status,
        pair_report,
    ...
    logger.info("Dataset health monitoring pipeline completed successfully.")
